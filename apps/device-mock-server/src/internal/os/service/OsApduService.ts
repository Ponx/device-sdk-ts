...
export class OsApduService {
   ...
   * advertises an MCU that keeps `shouldFlashMCU` false. Falls back to
   * {@link DEFAULT_MCU_VERSION} when no resolver is wired or the lookup yields
   * nothing (offline, or the device is already on the latest firmware), so the
